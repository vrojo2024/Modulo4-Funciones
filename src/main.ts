...
const entradaManual = document.getElementById("entrada-manual") as HTMLInputElement;//Creo constantes y obtengo elemento del html por id entradaManual tratándolo como un elemento de tipo HTMLInputElenment
...
function llamaTurno (turno: number) {//Actualizo el número de turno mostrado en la página. 
    ...
    if (turnoDisplay) //Esta linea comprueba si la variable turnoDisplay existe y no es null ni undefined
...
function numeroManual (manTurno: string) { //Establece un turno manual que es el que ingresó el usuario, de tipo string
    ...
    if (turnoManual) //Si el elemento turnoManual existe en la página y no es null ni undefined,
...
if (turnoAnterior) {//Si el elemento turnoAnterior  existe en la página y no es null ni undefined,
...
if (reseteoTurno) { // Si el elemento reseteoTurno existe en la página y no es null ni undefined,
...
if (turnoSiguiente) {// Si el elemento turnoSiguiente existe en la página y no es null ni undefined,
...
if (entradaManual){ // Si el elemento ntradaManual existe en la página y no es null ni undefined,
